...
class TestToolProgress:
    ...
    def test_terminal_erases_the_line_on_a_carriage_return_when_it_is_a_tty(self):
        class Tty(io.StringIO):
            def isatty(self):
                return True
        out = Tty()
        ToolProgress(Terminal(out=out, colour=False)).feed("tool_name", "Edit")
        assert out.getvalue() == "\r\033[K⚙ Edit"
