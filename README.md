...
| `--max-continues` | `3` | Times in a row a capped turn is continued (with `--cont`, and always for subagents) before the run ends; the last capped turn is salvaged. |
...
| `--edit` | off | Offer the `Edit` tool: one exact-string replacement or several in one file in one call, applied in order and all or none (every failure is reported, nothing is written). Subagents always get it. |
...
- **Re-read guard.** A looking call answered twice in the task — this turn and the capped turns it was continued from, checkpoints included — with nothing written or edited since gets a notice instead of the result. The count resets at an acting call (`Tool.acting`: Write, Edit, a delegate); Bash is registered as looking (`acts=False`), so a `grep` or `sed -n` neither resets the count nor escapes it. Memory calls are never counted.
- **Repeat guard.** Three identical rounds with identical results end the turn (stop `repeat`); memory calls are left out of the comparison, so rewriting a note or retrying a refused memory write is bounded by the round cap instead of ending the turn.
- **Continue bound.** A capped turn is continued at most `--max-continues` times in a row; the last one nothing will continue is salvaged like an overflow when no operator is present, so a subagent that keeps re-gathering after each checkpoint returns its record instead of looping.
- **Round budget.** One round's results must fit once a checkpoint has folded everything before them (the checkpoint keeps the last round whole): the window less the room a completion needs, the re-sent prompt (system prompt, tool schemas, memory block) and the checkpoint's share. Calls run in order while one more result at the tool cap still fits; the rest are answered `Not run: this round's results already take ~U of the ~B tokens one round may hold…`, which tells the model what the context can still take. The first call and memory calls always run.
...
- **Fold functions.** Once a call ran, the round echoes it with its long argument removed and a `folded` note in its place, a key the schema does not have: a Write's content over 120 chars becomes `folded: "N characters written; Read the file to see it"`, an Edit's edits over 240 chars of old and new strings together become their count and sizes, a `scratchpad_write` value becomes `folded: "N characters, shown in the scratchpad block"`, and a delegate brief over 400 chars keeps the head of its task. The argument is removed rather than replaced because the model copies the shape of its own echoed calls: a copied fold is a rejected call, never a marker written as a value.
